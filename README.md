...
## What's New (Past 36 Hours)

**Major additions and fixes**:
...
- **Monitoring stack** — Prometheus + Grafana now cover the full pipeline (see [Monitoring](#monitoring)):
  - Flink's built-in Prometheus reporter (job uptime, checkpoint duration/failures, records in/out)
  - Kafka broker metrics via a JMX exporter + consumer-group lag via `kafka-exporter` (two exporters, different metric families — documented below)
...
**Dashboards** (`compose/grafana/provisioning/dashboards/`): `webhook-dashboard`, `flink-dashboard` (job health/checkpointing), `kafka-dashboard` (lag + throughput), `generator-dashboard` (throughput/errors/backpressure).
